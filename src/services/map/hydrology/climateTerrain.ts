...
export function buildWaterInfluence(cells: TMapCell[]): Float32Array {
  ...
  for (let iteration = 0; iteration < HYDROLOGY_CONFIG.waterInfluenceIterations; iteration += 1) {
    for (let cellIndex = 0; cellIndex < cells.length; cellIndex += 1) {
      ...
      let total = waterInfluence[cellIndex] * HYDROLOGY_CONFIG.waterInfluenceSelfWeight;
      for (const neighborId of cell.neighbors) {
        total += waterInfluence[neighborId];
      }

      nextInfluence[cellIndex] =
        total / (cell.neighbors.length + HYDROLOGY_CONFIG.waterInfluenceSelfWeight);
    }
...
export function getRainShadow(cell: TMapCell, cells: TMapCell[]) {
  ...
  for (const neighborId of cell.neighbors) {
    ...
    if (elevationDelta > HYDROLOGY_CONFIG.rainShadowMinElevationDelta) {
...
export function getTerrainBand(
  ...
  if (cell.elevation < seaLevel - HYDROLOGY_CONFIG.deepWaterOffset) return 'deep-water';
  if (cell.elevation < seaLevel) return 'shallow-water';
  if (cell.elevation < seaLevel + HYDROLOGY_CONFIG.coastBand) return 'coast';

  const inValley =
    cell.isRiver ||
    (relief < HYDROLOGY_CONFIG.valleyReliefThreshold &&
      precipitation > HYDROLOGY_CONFIG.valleyPrecipitationMin);

  if (
    temperature < HYDROLOGY_CONFIG.tundraTemperatureMax ||
    cell.elevation > HYDROLOGY_CONFIG.tundraElevationMin
  ) {
    return 'tundra';
  }

  const elevationAboveSea = cell.elevation - seaLevel;
  const isNewlyEmergedLand = elevationAboveSea < HYDROLOGY_CONFIG.emergedLandBand;

  if (
    relief > HYDROLOGY_CONFIG.reliefMountainMin &&
    (cell.elevation > HYDROLOGY_CONFIG.mountainElevationMin || isNewlyEmergedLand)
  ) {
    return 'mountains';
  }

  if (
    relief > HYDROLOGY_CONFIG.reliefHillMin &&
    (cell.elevation > HYDROLOGY_CONFIG.hillElevationMin || isNewlyEmergedLand)
  ) {
    return 'hills';
  }

  if (
    cell.elevation > HYDROLOGY_CONFIG.plateauElevationMin &&
    relief < HYDROLOGY_CONFIG.plateauReliefMax &&
    precipitation > 0.22 &&
    precipitation < 0.58
  ) {
    return 'plateau';
  ...
  if (cell.elevation > HYDROLOGY_CONFIG.mountainElevationMin) return 'mountains';
  if (cell.elevation > HYDROLOGY_CONFIG.hillElevationMin) return 'hills';
  if (inValley) return 'valley';

  if (
    temperature > HYDROLOGY_CONFIG.desertTemperatureMin &&
    precipitation < HYDROLOGY_CONFIG.desertPrecipitationMax &&
  ...
  if (
    precipitation > HYDROLOGY_CONFIG.swampPrecipitationMin &&
    cell.elevation < HYDROLOGY_CONFIG.swampElevationMax &&
    relief < HYDROLOGY_CONFIG.swampReliefMax
  ) {
    return 'swamp';
  }

  if (precipitation > HYDROLOGY_CONFIG.forestPrecipitationMin && temperature > 0.22) {
    return 'forest';
  }
  if (precipitation < 0.2 && temperature > 0.56 && rainShadow > 0.35) return 'desert';
  if (
    cell.elevation > HYDROLOGY_CONFIG.mountainElevationMin - 0.04 &&
    precipitation < 0.34 &&
    temperature > 0.42
  ) {
    return 'volcanic';
