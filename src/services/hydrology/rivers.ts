...
function validateRivers(
    ...
    for (let cellIndex = 0; cellIndex < cells.length; cellIndex += 1) {
      ...
      while (cursor >= 0 && cursor < cells.length && visited[cursor] === 0 && mask[cursor] === 1) {
        ...
        if (cells[next].isLake) {
          ...
          endType = lakeSize >= HYDROLOGY_CONFIG.largeLakeMin ? 'large-lake' : 'invalid';
          break;
        }
      ...
      const validPlainSource =
        sourceCell.terrain === 'plains' && flow[cellIndex] >= HYDROLOGY_CONFIG.plainRiverSrcFlowMin;
      const validTundraSource =
        sourceCell.terrain === 'tundra' &&
        flow[cellIndex] >= HYDROLOGY_CONFIG.tundraRiverSrcFlowMin;
      const validSource =
        sourceCell.elevation >= minSourceElevation ||
        sourceLakeSize >= HYDROLOGY_CONFIG.largeLakeMin ||
        validPlainSource ||
        validTundraSource;
      const validEnd =
        endType === 'sea' ||
  ...
  addCandidates(
    isRiver,
    HYDROLOGY_CONFIG.riverSrcElevMin,
    HYDROLOGY_CONFIG.riverMinLen,
    HYDROLOGY_CONFIG.riverMinFlow
  );

  if (candidates.length < HYDROLOGY_CONFIG.riverMinCount) {
    ...
    addCandidates(
      relaxedMask,
      HYDROLOGY_CONFIG.riverSrcElevMin - HYDROLOGY_CONFIG.relaxedRiverSrcElevDrop,
      HYDROLOGY_CONFIG.relaxedRiverMinLen,
  ...
  const isLargeLand = landCellCount >= HYDROLOGY_CONFIG.riverLandThreshold;
  const targetLarge = isLargeLand
    ? HYDROLOGY_CONFIG.largeRiverMaxOnLargeLand
    : HYDROLOGY_CONFIG.largeRiverCountOnSmallLand;
  const minLargeBase = isLargeLand ? HYDROLOGY_CONFIG.largeRiverMinOnLargeLand : 1;
  const minLarge = hasLargePlains ? Math.max(2, minLargeBase) : minLargeBase;
  let targetSmall = isLargeLand
    ? HYDROLOGY_CONFIG.smallRiverTargetOnLargeLand
    : HYDROLOGY_CONFIG.smallRiverTargetOnSmallLand;
  ...
  const preferredMinLength = Math.max(
    HYDROLOGY_CONFIG.riverMinLen + 3,
    HYDROLOGY_CONFIG.relaxedRiverMinLen + 4
  );
  ...
  for (const candidate of preferredRemaining) {
    ...
    const maxOverlapRatio =
      candidate.joinsRiverCellId !== null ? HYDROLOGY_CONFIG.tribMaxOverlap : 0.8;
...
function addInlandPlainTributaries(cells: TCell[], flow: Float32Array, downstream: Int32Array) {
  ...
  const candidateSources = Array.from({ length: cells.length }, (_, cellIndex) => cellIndex)
    .filter((cellIndex) => {
      ...
      if (regionSizes[currentRegionId] < HYDROLOGY_CONFIG.vLargePlainMin) return false;
      return flow[cellIndex] >= HYDROLOGY_CONFIG.inlandPlainTribMinFlow;
    })
    .sort((left, right) => flow[right] - flow[left]);

  const selectedByRegion = new Map<number, number>();
  const minLength = HYDROLOGY_CONFIG.inlandPlainTribMinLen;
  ...
  for (const sourceId of candidateSources) {
    ...
    if (selectedCount >= HYDROLOGY_CONFIG.inlandPlainTribMaxPerPlain) continue;
...
function extendRiversTowardHighlands(
  ...
  for (let cellIndex = 0; cellIndex < cells.length; cellIndex += 1) {
    ...
    for (let step = 0; step < HYDROLOGY_CONFIG.highlandExtendMaxSteps; step += 1) {
      visited.add(cursor);
      ...
      for (const neighborId of cells[cursor].neighbors) {
        ...
        if (flow[neighborId] < HYDROLOGY_CONFIG.highlandExtendMinFlow) continue;
      ...
      if (
        ...
        cells[bestNeighborId].elevation >= HYDROLOGY_CONFIG.highlandTargetElevMin
      ) {
