...
function getTerrainFitness(
  ...
  if (terrain === 'plateau') {
    ...
    return 0.58 + (relief < HYDROLOGY_CONFIG.plateauReliefCap ? 0.35 : -0.1);
  }
...
function clusterLandTerrains(cells: TCell[], seaLevel: number) {
  ...
  for (const region of smallRegions) {
    ...
    const borderTerrains = new Set<TTerrain>();
  ...
  for (let iteration = 0; iteration < HYDROLOGY_CONFIG.region.smoothPasses; iteration += 1) {
    ...
    for (let cellIndex = 0; cellIndex < cells.length; cellIndex += 1) {
      ...
      if (dominantCount < HYDROLOGY_CONFIG.region.dominantMin || dominantTerrain === cell.terrain) {
        continue;
      }

      const relief = cell.elevation - getAvgNeighbor(cell, cells);
      const currentScore =
        getTerrainFitness(cell.terrain, cell, seaLevel, relief) +
        (counts.get(cell.terrain) || 0) * HYDROLOGY_CONFIG.region.currentBonus;
      const dominantScore =
        getTerrainFitness(dominantTerrain, cell, seaLevel, relief) +
        dominantCount * HYDROLOGY_CONFIG.region.dominantBonus;

      if (dominantScore > currentScore + HYDROLOGY_CONFIG.region.switchGap) {
...
function rebalanceTerrain(cells: TCell[], terrainBalance: TTerrainBalance) {
  ...
  function getShare(terrain: TTerrain) {
  ...
  function trimExcessToPlains(source: TTerrain, maxShare: number, score: (cell: TCell) => number) {
...
function antiAliasTerrains(cells: TCell[]) {
  for (let pass = 0; pass < HYDROLOGY_CONFIG.antiAlias.passes; pass += 1) {
    ...
    for (let cellIndex = 0; cellIndex < cells.length; cellIndex += 1) {
      ...
      if (sameCount > HYDROLOGY_CONFIG.antiAlias.isolatedMax) continue;
      ...
      if (dominantCount < HYDROLOGY_CONFIG.antiAlias.dominantMin) continue;
      nextTerrains[cellIndex] = dominantTerrain;
    }
...
function joinSmallZones(cells: TCell[], seaLevel: number) {
  ...
  for (let cellIndex = 0; cellIndex < cells.length; cellIndex += 1) {
    ...
    if (region.length >= (TERRAIN_CONFIG[terrain].clusterMin || 0)) continue;
    ...
    for (const regionCellId of region) {
      ...
      const borderCounts = new Map<TTerrain, number>();
      for (const neighborId of cell.neighbors) {
        if (regionSet.has(neighborId)) continue;
        const neighborTerrain = cells[neighborId].terrain;
        if (isHydrologyWaterTerrain(neighborTerrain)) continue;
